...
- Decide `enableAuthorizationChecks` per action: pass `false` only when a declarative class-level policy fully covers access (as above); leave the default `true` when entity-level (row) authorization applies — a resource authorization handler must then exist (`dotnet-scaffold-resource-auth-handler`).
