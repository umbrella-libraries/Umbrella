...
**Prerequisite:** `dotnet-scaffold-api-data-service-controller` must have run first. The service interface (`IManage<Name>Service`) and server controller service (`Manage<Name>ControllerService`) must already exist.
...
Find the existing `AddScoped<IManage<Name>Service, Manage<Name>ControllerService>()` line added by `dotnet-scaffold-api-data-service-controller` and change it to `ReplaceScoped`:
