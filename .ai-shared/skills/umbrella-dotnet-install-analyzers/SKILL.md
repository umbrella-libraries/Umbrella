...
description: 'Install the Umbrella Roslyn analyzer packages (Umbrella.Analyzers, Umbrella.DataAccess.Analyzers, Umbrella.Utilities.Mapping.Mapperly.Analyzers, Umbrella.WebUtilities.DynamicImage.Analyzers) into a consuming .NET repository with the correct per-package scope.'
---
...
Each package has a different correct scope. Do not install all four the same way.
...
3. **Check what is already installed.** Grep all `.csproj`/`.props` files for each of the four package IDs. Skip any that are already present at the correct version and scope; report a version mismatch rather than silently overwriting it.
...
## Step 1 — Install the global analyzers

In `Directory.Packages.props`, add both packages to the existing unconditioned `<ItemGroup>` (next to `Microsoft.CodeAnalysis.Analyzers` etc.), using the version resolved in Discovery step 1:
