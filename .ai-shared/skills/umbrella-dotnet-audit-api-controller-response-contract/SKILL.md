---
name: umbrella-dotnet-audit-api-controller-response-contract
...
- Scalar foreign-key properties mapped from request models, their corresponding relational constraints, and whether the action prevalidates the related entity — an unvalidated nonexistent key can deterministically reach the catch-all `500` during persistence.
...
- Host facts (from `umbrella-dotnet-audit-aspnetcore-integration-test-readiness` if already run): `UseUmbrellaPropagateClaimsPrincipal` present, validation failure status code (below), environment name strategy.
