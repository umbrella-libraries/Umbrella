...
2. **If central config is missing or drifted** - run/use `dotnet-standardize-test-projects` first. If that skill is not available, apply the documented central Umbrella test pattern before scaffolding. Do not compensate by adding runner properties or shared test packages to the architecture test project.
