...
This happens **before the action executes**. If the test host does not call `ConfigureUmbrellaApiBehaviorOptions()`, all of these cases fall back to the ASP.NET default `400`, and every `422` assertion in the contract becomes untestable — treat this as a mandatory prerequisite. If the application overrides `validationFailureStatusCode`, generated tests must assert that configured value wherever this guidance says `422`.
