...
| UA012 | Model properties must use the required keyword                        | Public instance settable properties on model/QueryResult types must use the `required` modifier. Static, non-public, getter-only, directly marked concrete input-model, and model-interface properties are excluded; individual properties can use `[UmbrellaAllowNonRequiredProperty]`. |
| UA013 | Model properties must have getter and be init-only                    | Public instance properties on model/QueryResult types must have a getter and normally use `{ get; init; }`. Getter-only properties are valid. Directly marked concrete input models may use `set`; model interfaces are excluded; and a property can use `[UmbrellaAllowMutableProperty]` where a lifecycle hook genuinely needs to assign it. Mutable `IConcurrencyStamp` properties are governed by UA023. |
...
| Attribute | Effect |
|-----------|--------|
| `[UmbrellaInputModel]` | On a concrete input model type, allows non-`required` properties (UA012) and `set` accessors (the mutability part of UA013), and identifies user-input types for UA015. It is not inherited and is invalid on abstract types. It does not suppress UA011, UA014, UA022, or UA013 for a missing getter. |
| `[UmbrellaAllowUnsealedModel("reason")]` | Allows one concrete model record class to remain unsealed because it is intentionally inherited. |
