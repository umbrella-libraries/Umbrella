...
2. Read 1–2 existing controller services in `Web\<AppName>.Web.Server\Services\Api\` to confirm the `UmbrellaRepositoryDataService` generic parameter order and project-specific patterns.
...
**File:** `Web\<AppName>.Web.Server\Services\Api\<Name>ControllerService.cs`
...
3. `<Name>ControllerService` is `public class`, inherits `UmbrellaRepositoryDataService` with 11 type params, uses `IHostEnvironment` (not `IWebHostEnvironment`).
