---
name: umbrella-dotnet-migrate-repo-controller-to-data-service
...
- If the client data interface still uses the `...Repository` naming convention, run `umbrella-dotnet-rename-client-repository-to-service` first to rename it to `...Service` and move it to `Services\Abstractions\`.
...
**If already created by `umbrella-dotnet-rename-client-repository-to-service`:** skip this step.
...
**If the client service interface already exists** (created by `umbrella-dotnet-rename-client-repository-to-service` or previously scaffolded by `umbrella-dotnet-scaffold-client-data`):
```csharp
