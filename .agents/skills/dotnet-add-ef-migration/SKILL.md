...
```powershell
powershell -ExecutionPolicy Bypass -File .agents\skills\dotnet-add-ef-migration\scripts\Invoke-AddEfMigration.ps1 -MigrationName 1.0.20
```

Add a migration (explicit projects):

```powershell
powershell -ExecutionPolicy Bypass -File .agents\skills\dotnet-add-ef-migration\scripts\Invoke-AddEfMigration.ps1 `
