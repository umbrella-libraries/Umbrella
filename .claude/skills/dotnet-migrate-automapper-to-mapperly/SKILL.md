---
name: dotnet-migrate-automapper-to-mapperly
...
See `dotnet-scaffold-mapperly-factories` for the full interface table. Quick reference:
...
Group related AutoMapper profiles by entity/feature and write one `<Feature>Mappers.cs` file per group, following `dotnet-scaffold-mapperly-factories` for file location and naming. Apply the conversion reference above for each `CreateMap` pair.

All rules from `dotnet-scaffold-mapperly-factories` apply: `public partial class`, `[Mapper]` on each class, `[MapperIgnoreTarget]` on private partial methods only, `Guard.IsNotNull(source)` in every public wrapper with a body.
...
Follow `dotnet-scaffold-mapperly-factories` Step 4. The `[assembly: UmbrellaMapperlyCatalogReference(typeof(...))]` attribute goes in the consuming project's `IServiceCollectionExtensions.cs`.
...
- Read `dotnet-scaffold-mapperly-factories` before writing any mapper files — all its rules apply here.
