...
2. **`ValidationProblem(ModelState)` from an action body is `400`**, not `422`. The configured `validationFailureStatusCode` (default `422`) applies only to pre-action model binding/annotation failures. Identity-style flows that copy `IdentityResult` errors into model state therefore produce `400`.
