...
1. Read `.claude\skills/umbrella-frontend-audit-toolchain/SKILL.md` and inventory the requested repositories. Compare multiple targets first; do not overwrite one app with another's configuration.
...
3. Read the applicable focused SKILL.md files from `.claude\skills` before applying them. Coordinate shared edits once so workflows do not overwrite each other:
